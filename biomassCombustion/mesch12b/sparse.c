...
#endif
{
   ...
   /* else */ if ( idx < -1 )
   {
      ...
      {
	 r->len = r->maxlen;
	 new_len = maxMesch(2*r->maxlen+1,5);
	 if (mem_info_is_on()) {
...
#endif
{
   ...
   for ( i = 0; i < A->m; i++ )
   {
      row1 = &(A->row[i]);
      row2 = &(out->row[i]);
      if ( ! (row2->elt=NEW_A(maxMesch(row1->len,3),row_elt)) )
	error(E_MEM,"sp_copy");
      else if (mem_info_is_on()) {
	 mem_bytes(TYPE_SPMAT,0,maxMesch(row1->len,3)*sizeof(row_elt));
      }
      row2->len = row1->len;
      row2->maxlen = maxMesch(row1->len,3);
      row2->diag = row1->diag;
