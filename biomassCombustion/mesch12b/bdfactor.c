...
#endif
{
   ...
   lb = A->lb = minMesch(n-1,lb);
   ub = A->ub = minMesch(n-1,ub);
...
#endif
{
   ...
   new_lb = A->lb = minMesch(new_lb,new_n-1);
   new_ub = A->ub = minMesch(new_ub,new_n-1);
...
#endif
{
   ...
   for (j=0; j < n; j++)
     for (i=minMesch(n1,j+lb),l=lb+j-i; i >= maxMesch(0,j-ub); i--,l++)
       A->me[i][j] = bmat[l][j];
...
#endif
{
   ...
   if (! in_situ) {
	 ...
	 **********************************/
	 ...
	 lbi = lb-i;
	 for (j=l-lb, jj=0, p=maxMesch(-lbi,0), pp = maxMesch(l-ub,0); j <= n1; 
	      j++,jj++,p++,pp++) {
	    in_v[l][pp] = in_v[i][p];
	    in_v[i][jj] = in_v[l][j];
	 }
	 for (  ; p <= n1-maxMesch(lbi,0); p++,pp++)
	   in_v[l][pp] = in_v[i][p];
      }
      
      if (lub%2 == 0) { /* shift only */
	 i = lub/2;
	 for (j=maxMesch(i-lb,0), jj=0; jj <= n1-ub+i; j++,jj++) 
	   in_v[i][jj] = in_v[i][j];
      }
   }
      ...
      for (i=0, l=lub; i < (lub+1)/2; i++,l--) {
	 ubi = i-ub;
	 for (j=n1-maxMesch(lb-l,0), jj=n1-maxMesch(-ubi,0), p=n1-lb+i, pp=n1;
	      p >= 0; j--, jj--, pp--, p--) {
	    in_v[i][jj] = in_v[l][j];
	    in_v[l][pp] = in_v[i][p];
	 }
	 for (  ; jj >= maxMesch(ubi,0); j--, jj--)
	   in_v[i][jj] = in_v[l][j];
      }

      if (lub%2 == 0) {  /* shift only */
	 i = lub/2;
	 for (j=n1-lb+i, jj=n1-maxMesch(ub-i,0); j >= 0; j--, jj--) 
	    in_v[i][jj] = in_v[i][j];
      }
   }
...
#endif
{
  ...
  for ( j = 0; j < x->dim; j++ )
    for ( i = maxMesch(j-bA->ub,0); i <= j+bA->lb && i < x->dim; i++ )
...
#endif
{
  ...
  OUT = bd_copy(A,OUT);
  OUT = bd_resize(OUT,maxMesch(A->lb,B->lb),maxMesch(A->ub,B->ub),A->mat->n);
...
/* bdLUfactor -- gaussian elimination with partial pivoting
   -- on entry, the matrix A in band storage with elements 
      ...
      maxMesch(0,j-lb) <= i <= minMesch(A->n-1,j+ub);
...
#endif
{
   ...
   bA = bd_resize(bA,lb,minMesch(n1,lub),n);
   bA_v = bA->mat->me;
   ...
   for ( k=0; k < n1; k++ )
   {
      k_end = maxMesch(0,lb+k-n1);
      k_lub = minMesch(k+lub,n1);
...
#endif
{
   ...
   for (i=n-2; i >= 0; i--) {
      c = x->ve[i];
      for (j=minMesch(n1,i+ub), l=lb+j-i; j > i; j--,l--)
	c -= bA_v[l][j]*x->ve[j];
      x->ve[i] = c/bA_v[lb][i];
   }
...
#endif
{
   ...
   for (k=0; k < n; k++) {    
      ...
      /* matrix D */
      c = Av[lb][k];
      for (j=maxMesch(0,-lbkm), jk=lbkm+j; j < k; j++, jk++) {
	 cc = Av[jk][j];
	 c -= Av[lb][j]*cc*cc;
      }
      if (c == 0.0)
	error(E_SING,"bdLDLfactor");
      ...
      for (i=minMesch(n1,lbkp), ki=lbkp-i; i > k; i--,ki++) {
	 c = Av[ki][k];
	 for (j=maxMesch(0,i-lb), ji=lb+j-i, jk=lbkm+j; j < k;
	      j++, ji++, jk++)
...
#endif
{
   ...
   for (i=1; i < n; i++) {
      ilb = i-lb;
      c = b->ve[i];
      for (j=maxMesch(0,ilb), l=j-ilb; j < i; j++,l++)
	c -= Av[l][j]*x->ve[j];
      x->ve[i] = c;
   }
   ...
   for (i=n-2; i >= 0; i--) {
      ilb = i+lb;
      c = x->ve[i];
      for (j=minMesch(n1,ilb), l=ilb-j; j > i; j--,l++)
	c -= Av[l][i]*x->ve[j];
      x->ve[i] = c;
   }
...
VEC *x, *out;
{
  ...
  for (i=0; i<n; i++, start_idx--, end_idx--) {
    j = maxMesch(0, start_idx);
    k = maxMesch(0, -start_idx);
    j_end = minMesch(m, end_idx);
    x_ve = x->ve + k;
    sum = 0.0;	     
    for (; j < j_end; j++, k++)
      sum += A_me[j][k] * *x_ve++;
