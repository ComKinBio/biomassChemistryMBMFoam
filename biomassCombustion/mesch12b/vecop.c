...
#ifndef ANSI_C
double	v_minMesch(x, min_idx)
VEC	*x;
int	*min_idx;
#else
double	v_minMesch(const VEC *x, int *min_idx)
#endif
{
    int		i, i_min;
    Real	min_val, tmp;
...
#ifndef ANSI_C
double	v_maxMesch(x, max_idx)
VEC	*x;
int	*max_idx;
#else
double	v_maxMesch(const VEC *x, int *max_idx)
#endif
{
    int		i, i_max;
    Real	max_val, tmp;
